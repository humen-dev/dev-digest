...
describe('RepoIntel facade — degraded contract (flag off)', () => {
  ...
  it('getFileRank / getSymbolsInFiles / getConventionSamples / getRankedPaths / getTopFilesByRank / getCriticalPaths → []', async () => {
    const svc = buildDegradedService({ flag: false });
    ...
    await expect(svc.getRankedPaths('r1', 50)).resolves.toEqual([]);
